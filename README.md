...
**Clears are delivered through a durable log, not pub/sub.** Redis pub/sub is fire-and-forget — a pod that's disconnected when a clear is published never receives it, and reconnecting doesn't replay it. So a refresh is appended to `inmem:refreshlog:<service>` (a Redis Stream capped at 1000 entries, 24h TTL), and every pod of that service reads from it with a blocking `XREAD`.

Each pod keeps its own cursor at `inmem:cursor:<service>:<pod>`, **committed only after its app has actually cleared**. That gives at-least-once delivery: a sidecar that drops its connection — or restarts outright — resumes exactly where it left off. If the app rejects a clear, the cursor stays put and the clear is retried until it lands, so a temporarily unhealthy app doesn't lose the invalidation. Retries are bounded (10 rounds), after which the sidecar logs an error and moves on, so one clear the app permanently rejects can't wedge every clear behind it.

This is deliberately **not** a consumer group. A group distributes each entry to exactly one member, so a single shared group would have one pod clearing while every other pod kept serving stale cache. Per-pod cursors give fan-out with no group metadata to garbage-collect: a dead pod's cursor simply expires.

A freshly started pod begins at the tail rather than replaying history its cache never held. Pub/sub is still used for request/response — reading a value from a specific pod, and collecting per-pod refresh confirmations — where a timeout is the wanted behavior.

> **Rolling upgrade:** refreshes are still published to the old pub/sub channel so sidecars predating the log keep clearing during a rollout. Deploy the version that writes the log before the version that reads it. The publish can be dropped once every sidecar is on a log-reading build.
...
**Redis is only for coordination** — sidecar discovery, message routing, and key metadata. The persistent keys are pod liveness (60s TTL), key metadata (3-day TTL), the refresh log (24h TTL, capped at 1000 entries), and each pod's refresh cursor (24h TTL), all self-cleaning. Nothing piles up.
